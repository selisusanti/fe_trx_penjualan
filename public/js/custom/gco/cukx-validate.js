...
for (let i = 0; i < inputLowerOnly.length; i++) {
    inputLowerOnly[i].addEventListener("keypress", function (evt) {
        if (evt.which < 97 || evt.which > 122) evt.preventDefault();
    });
...
for (let i = 0; i < inputUpperOnly.length; i++) {
    inputUpperOnly[i].addEventListener("keypress", function (evt) {
        if (evt.which < 65 || evt.which > 90) evt.preventDefault();
    });
...
// Only number input allowed
var antiXssUrl = document.getElementsByClassName("anti-xss-url");

/**
 * ! " # ; < > / \ ^
 */
const xssFactorUrl = [33, 34, 35, 59, 60, 61, 62, 92, 94];

for (let i = 0; i < antiXssUrl.length; i++) {
    antiXssUrl[i].addEventListener("keypress", function (evt) {
        if (xssFactorUrl.includes(evt.which)) {
            evt.preventDefault();
        }
    });

    antiXssUrl[i].addEventListener("paste", function (evt) {
        var text = evt.clipboardData.getData("text");
        evt.preventDefault();
