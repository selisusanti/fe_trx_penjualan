...
infinityScroll = function (wrapperId, contentId, methodType, uri, datatype, data, preHitAjax, postHitAjax, changeSettings, requestSuccess, requestError, endOfRequest){
    ...
    function scroller(){
        ...
        if(document.getElementById(wrapperId).scrollTop + document.getElementById(wrapperId).offsetHeight + (document.getElementById(wrapperId).offsetHeight * 0.3) > document.getElementById(contentId).offsetHeight)
        {
    ...
    if(endOfRequest == false){
        if(document.getElementById(wrapperId).addEventListener)
            document.getElementById(wrapperId).addEventListener("scroll",scroller,false);
    }
}
